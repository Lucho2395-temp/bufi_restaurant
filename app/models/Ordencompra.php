...
class Ordencompra
{
    ...
    public function guardar_orden_compra($model){
        try{
            if(isset($model->id_orden_compra)){
                ...
                $sql = 'insert into orden_compra (id_solicitante, id_aprobacion, id_proveedor, id_sucursal, orden_compra_fecha_aprob, orden_compra_titulo, 
                        orden_compra_activo,orden_compra_numero, orden_compra_estado, orden_compra_fecha, orden_compra_codigo, orden_compra_usuario_recibido) 
                        values (?,?,?,?,?,?,?,?,?,?,?,1)';
                $stm = $this->pdo->prepare($sql);
                $stm->execute([
                    $model->id_solicitante,
    ...
    public function guardar_detalle_compra($model){
        try {
            $sql = 'insert into detalle_compra (id_orden_compra, id_recurso_sede, detalle_compra_cantidad,detalle_compra_precio_compra,
                    detalle_compra_total_pedido, detalle_compra_estado) values(?,?,?,?,?,?)';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([
                $model->id_orden_compra,
    ...
    public function listar_reporte_movimiento_guardado($mt){
        try{
            $sql = 'select * from reporte_movimiento where reporte_movimiento_mt = ?';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$mt]);
            $result = $stm->fetch();
        } catch (Exception $e){
    ...
    public function listar_detalle_orden_compra($id_orden_compra){
        try{
            $sql = 'select * from detalle_compra where id_orden_compra = ?';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$id_orden_compra]);
            $result = $stm->fetchAll();
        } catch (Exception $e){
            $this->log->insertar($e->getMessage(), get_class($this).'|'.__FUNCTION__);
            $result = [];
    ...
    public function actualizar_valor_merma($recurso_sede_merma,$id_recurso_sede){
        try {
            $sql = 'update recursos_sede set recurso_sede_merma = ? where id_recurso_sede = ? ';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$recurso_sede_merma, $id_recurso_sede]);
            $result = 1;
        } catch (Exception $e){
            $this->log->insertar($e->getMessage(), get_class($this).'|'.__FUNCTION__);
            $result = 2;
