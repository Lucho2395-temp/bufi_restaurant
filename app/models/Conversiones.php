...
class Conversiones
{
    ...
    public function listar_conversiones_x_id($id_recurso_sede){
        try{
            $sql = 'select * from conversiones where id_recurso_sede = ?';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$id_recurso_sede]);
            $result = $stm->fetch();
        } catch (Exception $e){
    ...
    public function recursos_sede($id_usuario){
        try{
            $sql = 'select * from recursos_sede rs 
    				inner join sucursal s on rs.id_sucursal = s.id_sucursal 
    				inner join negocios n on s.id_negocio = n.id_negocio
                    inner join recursos r on rs.id_recurso = r.id_recurso 
         			where rs.id_usuario_creacion = ?
         			';
            $stm = $this->pdo->prepare($sql);
