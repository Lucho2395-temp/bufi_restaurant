...
class Pedido
{
    ...
    public function listar_pedidos_pendientes(){
        try{
            $sql = "select * from comanda_detalle as cd
         			...
         			where cd.comanda_detalle_atencion = 0
         			limit 20";
            $stm = $this->pdo->prepare($sql);
            $stm->execute();
            $result = $stm->fetchAll();
    ...
    public function listar_cantidad_usada_recurso($id_r){
        try{
			$sql = 'SELECT SUM(recurso_movimientos_cantidad) AS total_cantidad FROM recursos_movimientos WHERE id_recurso_sede = ? and recurso_movimientos_estado = 1';
			$stm = $this->pdo->prepare($sql);
            $stm->execute([$id_r]);
            $result = $stm->fetch();
        } catch (Exception $e){
    ...
    public function listar_cantidad_usar($id_r){
        try{
			$sql = 'SELECT * FROM conversiones WHERE id_recurso_sede = ? order by id_recurso_sede desc limit 1';
			$stm = $this->pdo->prepare($sql);
            $stm->execute([$id_r]);
            $result = $stm->fetch();
        } catch (Exception $e){
    ...
    public function listar_cantidad_actual_recurso($id_r){
        try{
    ...
    public function listar_id_receta_x_producto($id){
        try{
    ...
    public function listar_detalle_receta($id){
        try{
    ...
    public function eliminar_comanda_detalle_($fecha_eliminacion,$id_usuario_eli,$id_comanda_detalle){
        try{
            $sql = 'update comanda_detalle set comanda_detalle_fecha_eliminacion = ?,id_usuario_eli = ?, comanda_detalle_estado = 0 where id_comanda_detalle = ?';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$fecha_eliminacion,$id_usuario_eli,$id_comanda_detalle]);
            return 1;
        } catch (Throwable $e){
            $this->log->insertar($e->getMessage(), get_class($this).'|'.__FUNCTION__);
            return 2;
    ...
    public function listar_pedidos_delivery($id_mesa){
        try{
            $sql = 'select * from comanda c inner join clientes c2 on c.id_cliente = c2.id_cliente inner join comanda_detalle cd on 
                    c.id_comanda = cd.id_comanda inner join mesas m on c.id_mesa = m.id_mesa
                    where c.id_mesa = ? and cd.comanda_detalle_estado_venta = 0 group by c.id_comanda';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$id_mesa]);
            $result = $stm->fetchAll();
        } catch (Exception $e){
    ...
    public function listar_detalle_x_comanda($id_comanda){
        try{
            $sql = 'select * from comanda c inner join comanda_detalle cd on c.id_comanda = cd.id_comanda 
                    inner join productos p on cd.id_producto = p.id_producto 
                    inner join grupos g on p.id_grupo = g.id_grupo
                    where cd.id_comanda = ? and cd.comanda_detalle_estado=1';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$id_comanda]);
            $result = $stm->fetchAll();
        } catch (Exception $e){
    ...
    public function listar_detalle_x_comanda_x_fecha($id_comanda,$fecha){
        try{
            $sql = 'select * from comanda c inner join comanda_detalle cd on c.id_comanda = cd.id_comanda inner join productos p 
                    ...
                    inner join personas p2 on u.id_persona = p2.id_persona where cd.id_comanda = ? and cd.comanda_detalle_fecha_registro=?';
            $stm = $this->pdo->prepare($sql);
    ...
    public function listar_busqueda_productos($parametro){
        try{
            $sql = "select * from productos p 
    				inner join grupos g on p.id_grupo = g.id_grupo 
    				inner join producto_precio pp on p.id_producto = pp.id_producto
                    where  p.producto_estado = 1 and pp.producto_precio_estado = 1 and p.producto_nombre like ?";
            $stm = $this->pdo->prepare($sql);
    ...
    public function listar_mesero_x_detalle_comanda($id_comanda_detalle){
        try{
            $sql = 'select * from comanda_detalle c inner join usuarios u on c.id_usuario = u.id_usuario inner join 
                    personas p on u.id_persona = p.id_persona where c.id_comanda_detalle = ? limit 1';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$id_comanda_detalle]);
            $result = $stm->fetch();
        } catch (Exception $e){
    ...
    public function buscar_id_eli($id){
        try{
