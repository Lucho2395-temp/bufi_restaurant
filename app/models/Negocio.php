...
class Negocio
{
    ...
    public function listar_sucursal_egresos($id){
        try {
            $sql = 'select * from sucursal s inner join negocios n on s.id_negocio = n.id_negocio inner join usuarios_sucursal us 
                    on s.id_sucursal = us.id_sucursal where us.id_usuario = ?';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$id]);
            $result = $stm->fetchAll();
        } catch (Exception $e){
