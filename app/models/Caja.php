...
class Caja
{
    ...
    public function jalar_turno(){
        try{
            $sql = 'select turno_nombre from caja c inner join turno t on c.id_turno = t.id_turno
                    where t.turno_estado = 1 and c.caja_estado = 1 limit 1';
            $stm = $this->pdo->prepare($sql);
            $stm->execute();
            $ret = $stm->fetch();
