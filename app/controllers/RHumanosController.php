...
class RHumanosController
{
    ...
    public function gestion_personal(){
        try{
            ...
            if(isset($_POST['parametro'])){
                ...
                $parametro = "";
                $person = $this->humanos->listar_busqueda_();
            }
    ...
    public function asistencia_personal(){
        try{
            ...
            if(isset($_GET['id'])){
                ...
                $jalar_turnos = $this->humanos->listar_turnos_asistencia($_GET['id']);
                $buscar_registro = $this->humanos->buscar_registro($fecha_hoy,$_GET['id']);
                $hora_tolerancia = $jalar_turnos->turno_hora_tolerancia;
                $hora_salida = $jalar_turnos->turno_hora_salida;
                $hora_sistema = date('H:i:s');
                if(!empty($buscar_registro)){
                    $respuesta = "Registre su salida por favor!";
                }else{
                    if($hora_sistema < $hora_tolerancia){
    ...
    public function asignar_turnos(){
        try{
            $this->nav = new Navbar();
            $navs = $this->nav->listar_menus($this->encriptar->desencriptar($_SESSION['ru'],_FULL_KEY_));
            $turno_1 = $this->humanos->listar_turno(1);
            $turno_2 = $this->humanos->listar_turno(2);
            $turno_3 = $this->humanos->listar_turno(3);
            $turno_4 = $this->humanos->listar_turno(4);
            $datos = false;
            if(isset($_POST['parametro'])){
    ...
    public function gestionar_turnos(){
        try{
            $this->nav = new Navbar();
            $navs = $this->nav->listar_menus($this->encriptar->desencriptar($_SESSION['ru'],_FULL_KEY_));
            $turnos = $this->humanos->listar_turnos();
            require _VIEW_PATH_ . 'header.php';
            require _VIEW_PATH_ . 'navbar.php';
            require _VIEW_PATH_ . 'humanos/gestionar_turnos.php';
            require _VIEW_PATH_ . 'footer.php';
        }catch (Throwable $e){
            //En caso de errores insertamos el error generado y redireccionamos a la vista de inicio
            $this->log->insertar($e->getMessage(), get_class($this).'|'.__FUNCTION__);
    ...
    public function ver_memo(){
        try{
            ...
            $pdf->Cell(50,0,'-------------------------------------------------------',0,1,'L',0);
            $pdf->Cell(100,5,"REYES VARGAS SHARON GIANELA",0,1,'L',0);
    ...
    public function guardar_asistencia(){
        try{
            ...
            $id = $_POST['id'];
            $asistencia_estado = 0;
            $result = $this->humanos->actualizar_asistencia($id,$fecha,$valor,$asistencia_estado);
        } catch (Throwable $e){
            $this->log->insertar($e->getMessage(), get_class($this).'|'.__FUNCTION__);
            $result = 2;
        }
    ...
    public function registrar_asistencia(){
        try{
            ...
            $model->asistencia_hora = date('H:i:s');
            $model->asistencia_tipo = 0;
    ...
    public function registrar_salida(){
        try{
            ...
            $asistencia_hora_salida = date('H:i:s');
            $fecha_hoy = date('Y-m-d');

            $result = $this->humanos->guardar_salida($asistencia_hora_salida,$fecha_hoy,$id_persona_turno);
    ...
    public function constancia_trabajo(){
        try{
            ...
            foreach ($empresas as $e){
                ...
                $pdf->Ln(10);
                $pdf->SetFont('Arial','',11);
                $pdf->Cell(180,5,"Se expide el presente certificado para los fines que estime conveniente.",0,2,'J',0);
                $pdf->Cell(180,5,"Iquitos, ".$day." de ".$mes." del ".$year,0,1,'R',0);
                $pdf->Ln(10);
                $pdf->Image(_SERVER_.'media/logo/firma.jpg',null,null,40);
                $pdf->Cell(65,0,'---------------------------------------------------------',0,1,'C',0);
                $pdf->Cell(100,5,'REYES VARGAS SHARON GIANELA',0,1,'L',0);
                $pdf->SetFont('Arial','B',9);
        ...
        try{
            ...
            if($periodo->periodo_contrato== null){
                ...
                if($periodo->id_contrato==2) {
                    ...
                    $pdf->SetStyle("p", "arial", "N", 9, "0, 0, 0", 0);
                    $pdf->SetStyle("h1", "arial", "BU", 11, "0, 0, 0", 0);
                    $pdf->SetStyle("h2", "arial", "U", 11, "0, 0, 0", 0);
                    $pdf->SetStyle("vb", "arial", "B", 0, "0, 0, 0");
                    $pdf->SetFont('Arial','BU',11);
                    $pdf->Cell(180,2,'CONTRATO DE TRABAJO SUJETO A MODALIDAD POR INICIO O INCREMENTO DE NUEVA ACTIVIDAD',0,1,'C',0);
                    $pdf->Ln('2');
                    $pdf->SetFont('Arial','',9);
                    $pdf->Cell(180,5,"Contrato Laboral N° ".$nro." - ".$year." - ".$_SESSION['nombre_empresa'],0,1,'C',0);
                    $pdf->Ln('5');
                    $txt="<p>Conste por el presente documento el Contrato de Trabajo a plazo fijo bajo la modalidad de 'Contrato por inicio o incremento de actividad' que celebran al amparo del Art. 57° de la Ley de Productividad y Competitividad Laboral aprobado por D.S N°003-97-TR y normas complementarias, de una parte la empresa <vb>".$_SESSION['nombre_empresa']."</vb> con R.U.C. Nº ".$_SESSION['ruc']." y domicilio fiscal en ".$periodo->empresa_domiciliofiscal.", con acreditación del Registro de la <vb>Micro y pequeña Empresa N° 0001488204-2017</vb> debidamente representada por <vb>REYES VARGAS SHARON GIANELA</vb> identificada con D.N.I. Nº <vb>71100292</vb>, a quien en adelante se le denominará simplemente <vb>EL EMPLEADOR</vb>; y de la otra parte <vb>".$persona->persona_nombre." ".$persona->persona_apellido_paterno." ".$persona->persona_apellido_materno."</vb> con D.N.I. Nº <vb>".$persona->persona_dni."</vb>, de nacionalidad <vb>".$persona->persona_nacionalidad."</vb>, domiciliado en ".$persona->persona_direccion." ,".$persona->persona_distrito.", ".$persona->persona_provincia.", ".$persona->persona_departamento.", a quien en adelante se le denominará simplemente <vb>EL TRABAJADOR</vb>; en los términos y condiciones siguientes:</p>";
                    $pdf->WriteTag(0, 5, $txt, 0, "J", 0, 0);
                    $pdf->Ln('5');
                    $txt="<p><vb>PRIMERO:</vb> EL EMPLEADOR, es una empresa, cuyo objeto social es la de <vb>SERVICIO DE RESTAURANTE Y CEVICHERIA</vb> y otros afines, que requiere de los servicios del TRABAJADOR dentro de la empresa, a fin de prestar servicio bajo la modalidad de contrato sujeto a modalidad por inicio o incremento de actividad, de conformidad con el Art. 56 del T.U.O del Decreto Legislativo N° 728.</p>";
                    $pdf->WriteTag(0, 5, $txt, 0, "J", 0, 0);
                    $pdf->Ln('5');
                    $txt="<p><vb>SEGUNDO:</vb> Por el presente documento,<vb>EL EMPLEADOR</vb> contrata a plazo fijo bajo la modalidad ya indicada, los servicios de EL TRABAJADOR quien desempeñará el cargo de <vb>GERENTE GENERAL</vb>, en relación con las causas objetivas señaladas en la cláusula anterior.</p>";
                    $pdf->WriteTag(0, 5, $txt, 0, "J", 0, 0);
                    $pdf->Ln('5');
                    $txt="<p><vb>TERCERO:</vb> El presente contrato rige desde el <vb>".$fecha_ini[2]."-".$fecha_ini[1]."-".$fecha_ini[0]."</vb>, fecha en que debe empezar sus labores EL TRABAJADOR hasta el <vb>".$fecha_fini[2]."-".$fecha_fini[1]."-".$fecha_fini[0]."</vb>, fecha en que termina su contrato. </p>";
                    $pdf->WriteTag(0, 5, $txt, 0, "J", 0, 0);
                    $pdf->Ln('5');
                    $txt="<p><vb>CUARTO:</vb> EL TRABAJADOR estará sujeto a un período de prueba la misma que inicia <vb>".$fecha_ini[2]."-".$fecha_ini[1]."-".$fecha_ini[0]."</vb> y concluye <vb>".$fecha_fini[2]."-".$fecha_fini[1]."-".$fecha_fini[0]."</vb>.</p>";
                    $pdf->WriteTag(0, 5, $txt, 0, "J", 0, 0);
                    $pdf->Ln('5');
                    $txt="<p><vb>QUINTO:</vb> EL TRABAJADOR cumplirá el horario de trabajo siguiente: De lunes a domingo de ".$periodo->turno_hora_entrada." a.m. horas a ".$periodo->turno_hora_salida." p.m. horas, tomando un día de descanzo a la semana.</p>";
                    $pdf->WriteTag(0, 5, $txt, 0, "J", 0, 0);
                    $pdf->Ln('5');
                    $txt="<p><vb>SEXTO:</vb> EL TRABAJADOR deberá cumplir con las normas propias del Centro de Trabajo, así como las contenidas en el Reglamento interno de Trabajo (en caso cuente con uno debidamente registrado ante la AAT) y en las demás normas laborales, y las que se impartan por necesidad del servicio en ejercicio de las facultades de administración de la empresa, de conformidad con el Art. 9° de la Ley de Productividad y Competitividad Laboral aprobado por D.S. N° 003-97-TR.</p>";
                    $pdf->WriteTag(0, 5, $txt, 0, "J", 0, 0);
                    $pdf->Ln('5');
                    $txt="<p><vb>SEPTIMO:</vb> <vb>EL EMPLEADOR</vb> abonará al TRABAJADOR la cantidad de <vb>S/. ".$periodo->periodo_total." ($resultado CON $dec/00 Nuevos Soles)</vb> como remuneración mensual, de la cual se deducirá las aportaciones y descuentos por tributos establecidos en la ley que le resulten de aplicación.</p>";
                    $pdf->WriteTag(0, 5, $txt, 0, "J", 0, 0);
                    $pdf->Ln('5');
                    $txt="<p><vb>OCTAVO:</vb> Queda entendido que EL EMPLEADOR no está obligado a dar aviso alguno adicional referente al término del presente contrato, operando su extinción en la fecha de su vencimiento conforme la claúsula tercera, Oportuniad en la cual se abonara al TRABAJADOR los beneficios sociales que le pudieran corresponder de acuerdo a ley.</p>";
                    $pdf->WriteTag(0, 5, $txt, 0, "J", 0, 0);
                    $pdf->Ln('5');
                    $txt="<p><vb>NOVENA:</vb> Este contrato queda sujeto a las disposiciones que contiene el TUO del D.Leg. N° 728 aprobado por D.S. N°003-97-TR Ley de Productividad y Competitividad Laboral, y demás normas legales que lo regulen o que sean dictadas durante la vigencia del contrato.</p>";
                    $pdf->WriteTag(0, 5, $txt, 0, "J", 0, 0);
                    $pdf->Ln('2');
                    ...
                    $pdf->WriteTag(0, 5, $txt, 0, "J", 0, 0);
                    $pdf->Ln('5');
                    $pdf->Image(_SERVER_.'styles/gaia/img/firma.jpg',null,null,40);
                    $pdf->Cell(50,0,'----------------------------------------------',0,0,'C',0);
                    $pdf->Cell(110,0,'-------------------------------------------------------',0,1,'R',0);
                    $pdf->Cell(100,5,'REYES VARGAS SHARON GIANELA',0,0,'L',0);
                    $pdf->Cell(80,5,$persona->persona_apellido_paterno." ".$persona->persona_apellido_materno." ".$persona->persona_nombre,0,1,'L',0);
                    $pdf->SetFont('Arial','B',9);
                    $pdf->Cell(100,5,'DNI : 71100292',0,0,'L',0);
                    $pdf->Cell(80,5,'DNI:'.$persona->persona_dni,0,1,'L',0);
...
// Text
    ...
    public function agregar_turnos(){
        //Código de error general
        $result = 2;
        //Mensaje a devolver en caso de hacer consulta por app
        $message = 'OK';
        try{
            ...
            if($ok_data) {
                ...
                $model->turno_hora_entrada = $_POST['turno_hora_entrada'];
                $model->turno_hora_tolerancia = $_POST['turno_hora_tolerancia'];
                $model->turno_hora_salida = $_POST['turno_hora_salida'];
                $model->turno_estado = 1;
                $model->turno_fecha_registro = date('Y-m-d H:i:s');
                $model->turno_usuario_registro = $this->encriptar->desencriptar($_SESSION['c_u'],_FULL_KEY_);

                $result = $this->humanos->agregar_turnos($model);
            }else {
                //Código 6: Integridad de datos erronea
                $result = 6;
                $message = "Integridad de datos fallida. Algún parametro se está enviando mal";
    ...
    public function agregar_turnos_editado(){
        //Código de error general
        $result = 2;
        //Mensaje a devolver en caso de hacer consulta por app
        $message = 'OK';
        try{
            ...
            if($ok_data) {
                ...
                $result = $this->humanos->agregar_turnos_editado($model);
            }else {
                //Código 6: Integridad de datos erronea
                $result = 6;
                $message = "Integridad de datos fallida. Algún parametro se está enviando mal";
