    ...
    public function guardar_orden(){
        ...
        try{
            ...
            if($ok_data) {
                ...
                $model->orden_compra_fecha_aprob = null;
...
//                $model->orden_compra_fecha_aprob = date('Y-m-d H:i:s');
                        ...
                        if(count_chars($contenido)>0){
                            ...
                            if(count($filas)>0){
                                for ($i=0;$i<count($filas)-1;$i++){
                                    ...
                                    $this->ordencompra->actualizar_valor_merma($recurso_sede_merma,$id_recurso_sede);
                                }
                            }
                        }
                    }
    ...
    public function aprobar_orden(){
        ...
        try{
			...
			if($result == 1){
				$mt = microtime(true);
				$result = $this->builder->save("reporte_movimiento",array("reporte_movimiento_tipo" => 1, "id_usuario" => 1, "reporte_movimiento_fecha" => date('Y-m-d H:i:s'), "reporte_movimiento_estado" => 1, "reporte_movimiento_mt" => $mt));
				if($result == 1){
					$reporte_movimiento_id = $this->ordencompra->listar_reporte_movimiento_guardado($mt);
					$detalle_orden_compra = $this->ordencompra->listar_detalle_orden_compra($id_orden_compra);
					foreach ($detalle_orden_compra as $d){
						$this->builder->save("reporte_movimiento_detalle",array(
							...
							"id_recurso_sede" => $d->id_recurso_sede,
							"reporte_movimiento_detalle_cantidad" => $d->detalle_compra_cantidad,
