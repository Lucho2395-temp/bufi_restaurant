...
class PedidoController
{
    ...
    public function pedidos_atencion(){
        try{
            $this->nav = new Navbar();
            $navs = $this->nav->listar_menus($this->encriptar->desencriptar($_SESSION['ru'],_FULL_KEY_));
            $id_usuario = $this->encriptar->desencriptar($_SESSION['c_u'],_FULL_KEY_);
			$pedidos_pendientes = $this->pedido->listar_pedidos_pendientes();
            
            require _VIEW_PATH_ . 'header.php';
            require _VIEW_PATH_ . 'navbar.php';
            require _VIEW_PATH_ . 'pedido/pedidos_atencion.php';
            require _VIEW_PATH_ . 'footer.php';
        }catch (Throwable $e){
            //En caso de errores insertamos el error generado y redireccionamos a la vista de inicio
            $this->log->insertar($e->getMessage(), get_class($this).'|'.__FUNCTION__);
...
//            $ultimo_valor_ = $ultimo_valor->id_comanda;
    ...
    public function ticket_comanda(){
        try{
            ...
            //$nombre_ticket = $detalle_comanda[0]->grupo_ticketera;
    ...
    public function guardar_comanda(){
        ...
        try{
            ...
            if($ok_data) {
                ...
                $model->comanda_fecha_registro = date('Y-m-d H:i:s');
                ...
                if($guardar_comanda == 1){
					...
					$mt_reporte_movimientio_tipo = microtime(true);
					$result = $this->builder->save("reporte_movimiento",array(
						"reporte_movimiento_tipo" => 2,
						"id_usuario" => 1,
						"reporte_movimiento_fecha" => date('Y-m-d H:i:s'),
						"reporte_movimiento_estado" => 1,
						"reporte_movimiento_mt" => $mt_reporte_movimientio_tipo
					));
                    ...
                    if(count_chars($contenido)>0){
                        ...
                        if(count($filas)>0){
                            for ($i=0;$i<count($filas)-1;$i++){
                                ...
                                $modelDSI->comanda_detalle_fecha_registro = date('Y-m-d H:i:s');
                                $modelDSI->comanda_detalle_estado = 1;
								
								$id_receta = $this->pedido->listar_id_receta_x_producto($celdas[0])->id_receta;
								$detalle_receta = $this->pedido->listar_detalle_receta($id_receta);
								
								if($detalle_receta){
									foreach ($detalle_receta as $dr){
										$usado = $this->pedido->listar_cantidad_usada_recurso($dr->id_recursos_sede); //ya he usado 500
										if($usado->totalcantidad != null){
											$cantidad_usar = $this->pedido->listar_cantidad_usar($dr->id_recursos_sede); //solo puedo usar 1000
											$cantidad_stock_actual = $this->pedido->listar_cantidad_actual_recurso($dr->id_recursos_sede);
											if($usado < $cantidad_usar){
												$vali = $dr->detalle_receta_cantidad + $usado;
												if($vali > $cantidad_usar){
													$result = $this->builder->update('recursos_sede',array('recurso_sede_stock' => $cantidad_stock_actual->recurso_sede_stock - 1,),array('id_recurso_sede' => $dr->id_recursos_sede));
													$result = $this->builder->save('recursos_movimientos',array('id_recurso_sede' => $dr->id_recursos_sede, 'recurso_movimientos_cantidad' => $dr->detalle_receta_cantidad, 'recurso_movimientos_fecha' => date('Y-m-d H:i:s'),'recurso_movimientos_estado' => 1));
													$result = $this->builder->update('recursos_movimientos',array('recurso_movimientos_estado' => 0),array('id_recurso_sede' => $dr->id_recursos_sede));
													
													if($result == 1){
														$reporte_movimiento_id = $this->ordencompra->listar_reporte_movimiento_guardado($mt_reporte_movimientio_tipo);
														$this->builder->save("reporte_movimiento_detalle",array(
															"id_reporte_movimiento" => $reporte_movimiento_id->id_reporte_movimiento,
															"id_recurso_sede" => $dr->id_recursos_sede,
															"reporte_movimiento_detalle_cantidad" => $dr->detalle_receta_cantidad,
															"reporte_movimiento_detalle_estado" => 1,
														));
													}
											...
											$a = $dr->id_recursos_sede;
											$result = $this->builder->save('recursos_movimientos',array(
												'id_recurso_sede' => $dr->id_recursos_sede,
												'recurso_movimientos_cantidad' => $dr->detalle_receta_cantidad,
												'recurso_movimientos_fecha' => date('Y-m-d H:i:s'),
												'recurso_movimientos_estado' => 1
											));
                ...
                if($result == 1){
                    $result = $this->pedido->cambiar_estado_mesa($_POST['id_mesa']);
            ...
            if ($ok_data) {
                ...
                if(count_chars($contenido)>0){
                    ...
                    if(count($filas)>0){
                        ...
                        $nombre_ticket = $detalle_comanda[0]->grupo_ticketera;
                        $imp = 0;
                        $cant_pro = count($detalle_comanda);
                        foreach ($detalle_comanda as $no){
                            if($no->id_grupo == 2){
                                $imp++;
                            }
                        }
...
//                if($this->pedido->verificar_password($this->encriptar->desencriptar($_SESSION['_n'],_FULL_KEY_), $_POST['password'])) {
                if($ok_data) {
                    ...
                    $id_mesa = $_POST['id_mesa'];
                    $id_usuario_eli = $this->encriptar->desencriptar($_SESSION['c_u'],_FULL_KEY_);
                    $result = $this->pedido->eliminar_comanda_detalle_($fecha_eliminacion,$id_usuario_eli,$id_comanda_detalle);
                    if($result == 1){
                        $jalar_valor = $this->pedido->jalar_valor($id_comanda);
                        $nuevo_valor = $jalar_valor->total;
                        $actualizar_precio = $this->pedido->actualizar_nuevo_valor($id_comanda, $nuevo_valor);
    ...
    public function guardar_venta_(){
        //Código de error general
        $result = 2;
        //Mensaje a devolver en caso de hacer consulta por app
        $message = 'OK';
...
//                        $guardar_venta = $this->pedido->guardar_nota_venta($model);
    ...
    public function despachar_pedido_comanda(){
        $result = 2;
        $message = 'OK';
        try{
            $id = $_POST['id_comanda_detalle'];
			if($id){
				$result = $this->builder->update('comanda_detalle',array(
					'comanda_detalle_atencion' => 1, 
					'comanda_detalle_hora_atencion' => date('Y-m-d H:i:s'
					)),array(
