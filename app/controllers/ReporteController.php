...
class ReporteController
{
    ...
    public function reporte_general(){
        try {
            ...
            if(isset($_POST['enviar_fecha'])){
                ...
                $caja_ = $this->caja->datos_caja_($id_caja_numero);
                //$listar_egresos = $this->reporte->listar_egresos_descripcion($fecha_i,$fecha_f,$id_caja_numero);
                $datos = true;
            }
            require _VIEW_PATH_ . 'header.php';
            require _VIEW_PATH_ . 'navbar.php';
    ...
    public function reporte_atenciones(){
        try {
            $this->nav = new Navbar();
            $navs = $this->nav->listar_menus($this->encriptar->desencriptar($_SESSION['ru'], _FULL_KEY_));
			$reporte_atendidos = $this->reporte->listar_pedidos_atendidos();
            require _VIEW_PATH_ . 'header.php';
            require _VIEW_PATH_ . 'navbar.php';
            require _VIEW_PATH_ . 'reporte/reporte_atenciones.php';
            require _VIEW_PATH_ . 'footer.php';
        } catch (Throwable $e) {
            //En caso de errores insertamos el error generado y redireccionamos a la vista de inicio
            $this->log->insertar($e->getMessage(), get_class($this) . '|' . __FUNCTION__);
...
//			$reporte_atendidos = $this->reporte->listar_pedidos_atendidos();
            require _VIEW_PATH_ . 'header.php';
            require _VIEW_PATH_ . 'navbar.php';
            require _VIEW_PATH_ . 'reporte/reporte_entradas_salidas.php';
            require _VIEW_PATH_ . 'footer.php';
        } catch (Throwable $e) {
            //En caso de errores insertamos el error generado y redireccionamos a la vista de inicio
            $this->log->insertar($e->getMessage(), get_class($this) . '|' . __FUNCTION__);
    ...
    public function ventas_tipo_pago_pdf(){
        try{
            $fecha_hoy = date('Y-m-d');
            $fecha_fin = date('Y-m-d');
            //$pago_tipo = "";

            if($_GET['id_tipo_pago']!="" && $_GET['fecha_hoy']!="" && $_GET['fecha_fin']!=""){
                $fecha_hoy = $_GET['fecha_hoy'];
                $fecha_fin = $_GET['fecha_fin'];
                $pago_tipo = $_GET['id_tipo_pago'];
                $tipo_pago = $this->reporte->ventas_tipo_pago($pago_tipo,$fecha_hoy, $fecha_fin);
                $tipo_pago_ = $this->reporte->listar_tipos_pagos($pago_tipo);
                if($_GET['id_tipo_pago'] == 1){
                    $pago_nombre = 'Tarjeta';
                }elseif($_GET['id_tipo_pago'] == 3){
                    $pago_nombre = 'Efectivo';
                }else{
                    $pago_nombre = 'Transferencia';
                }
            }else if($_GET['fecha_hoy'] !="" && $_GET['fecha_fin'] !=""){
                $fecha_hoy = $_GET['fecha_hoy'];
                $fecha_fin = $_GET['fecha_fin'];
                $tipo_pago = $this->reporte->ventas_tipo_pago_fechas($fecha_hoy, $fecha_fin);
                $pago_nombre = 'TODOS';
            ...
            require _VIEW_PATH_ . 'reporte/ventas_tipo_pago_pdf.php';
        }catch (Throwable $e) {
            //En caso de errores insertamos el error generado y redireccionamos a la vista de inicio
            $this->log->insertar($e->getMessage(), get_class($this) . '|' . __FUNCTION__);
            echo "<script language=\"javascript\">alert(\"Error Al Mostrar Contenido. Redireccionando Al Inicio\");</script>";
    ...
    public function ticket_productos(){
        try{
            $fecha_filtro = $_POST['fecha_i'];
            $fecha_filtro_fin = $_POST['fecha_f'];
    ...
    public function excel_reporte_productos(){
        try{
            $fecha_i = $_GET['fecha_filtro'];
            $fecha_f = $_GET['fecha_filtro_fin'];
            $cajas_totales = $this->reporte->datos_por_apertura_caja_($fecha_i,$fecha_f);
            ...
            $nombre_excel = 'reporte_general' . '_' . $fecha_hoy;
            //creamos el archivo excel
            header( "Content-Type: application/vnd.ms-excel;charset=utf-8");
            header("Content-Disposition: attachment; filename=".$nombre_excel.".xls");
            require _VIEW_PATH_ . 'reporte/excel_reporte_productos.php';
        }catch (Throwable $e){
            //En caso de errores insertamos el error generado y redireccionamos a la vista de inicio
            $this->log->insertar($e->getMessage(), get_class($this).'|'.__FUNCTION__);
            echo "<script language=\"javascript\">alert(\"Error Al Mostrar Contenido. Redireccionando Al Inicio\");</script>";
    ...
    public function excel_solo_productos(){
        try {
            $fecha_i = $_GET['fecha_filtro'];
            $fecha_f = $_GET['fecha_filtro_fin'];
            $productos = $this->reporte->reporte_productos($fecha_i,$fecha_f);
            $fecha_hoy = date("d-m-y");
            $nombre_excel = 'reporte_productos' . '_' . $fecha_hoy;
            //creamos el archivo excel
            header( "Content-Type: application/vnd.ms-excel;charset=utf-8");
            header("Content-Disposition: attachment; filename=".$nombre_excel.".xls");
            require _VIEW_PATH_ . 'reporte/excel_solo_productos.php';
        }catch (Throwable $e){
            //En caso de errores insertamos el error generado y redireccionamos a la vista de inicio
            $this->log->insertar($e->getMessage(), get_class($this).'|'.__FUNCTION__);
            echo "<script language=\"javascript\">alert(\"Error Al Mostrar Contenido. Redireccionando Al Inicio\");</script>";
	...
	public function buscar_reporte_movimientos(){
		try{
			$result = [];
			$movimientos = $this->reporte->listar_reporte_movimientos($_POST['fecha_desde'],$_POST['fecha_hasta']);
			if($movimientos){
				foreach ($movimientos as $m){
					$detalles_ingreso = $this->reporte->listar_detalle_movimiento_ingreso($m->id_reporte_movimiento);
					$detalles_salida = $this->reporte->listar_detalle_movimiento_salida($m->id_reporte_movimiento);
					$medida_original = $this->reporte->listar_medida_original(!empty($detalles_ingreso) ? $detalles_ingreso->id_recurso_sede : $detalles_salida->id_recurso_sede);
					if($detalles_ingreso || $detalles_salida) {

						$recurso_nombre = !empty($detalles_ingreso) ? $detalles_ingreso->recurso_nombre : $detalles_salida->recurso_nombre;
						$ingreso = !empty($detalles_ingreso) ? (float)$detalles_ingreso->cantidad_total : 0;
						$salida = !empty($detalles_salida) ? (float)$detalles_salida->cantidad_total : 0;
						$conversion = !empty($detalles_ingreso) ? $detalles_ingreso->conversion_cantidad : $detalles_salida->conversion_cantidad;
						$medida_minima = !empty($detalles_ingreso) ? $detalles_ingreso->medida_codigo_unidad : $detalles_salida->medida_codigo_unidad;

						// Buscar si el recurso ya existe en el resultado
						$encontrado = false;
						foreach ($result as &$item) {
							if ($item->recurso === $recurso_nombre) {
								$item->ingreso += $ingreso;
								$item->salida += $salida;

								// Actualizar campos nulos si el nuevo valor no es nulo
								if ($item->conversion === null && $conversion !== null) {
									$item->conversion = $conversion;
								}
								if ($item->medida_minima === null && $medida_minima !== null) {
									$item->medida_minima = $medida_minima;
								}
						...
						// Si no se encontró, agregar nuevo elemento
						if (!$encontrado) {
							$result[] = (object)[
								'recurso' => $recurso_nombre,
								'ingreso' => $ingreso,
								'unidad_original' => $medida_original->medida_codigo_unidad,
								'salida' => $salida,
