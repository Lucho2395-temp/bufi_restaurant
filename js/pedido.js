...
function despachar_pedido_pendiente(id_comanda_detalle){
    var valor = true;
    var id_comanda_detalle = id_comanda_detalle;

    if(valor){
        var cadena = "id_comanda_detalle=" + id_comanda_detalle;
        $.ajax({
            type: "POST",
            url: urlweb + "api/Pedido/despachar_pedido_comanda",
            data: cadena,
            dataType: 'json',
            success: function (r) {
                switch (r.result.code) {
                    case 1:
                        respuesta('¡Pedido atendido!', 'success');
                        setTimeout(function () {
                            location.reload()
                        }, 300);
                        break;
                    case 2:
                        respuesta('Error al atender pedido', 'error');
                        break;
                    default:
                        respuesta('¡Algo catastrofico ha ocurrido!', 'error');
                        break;
